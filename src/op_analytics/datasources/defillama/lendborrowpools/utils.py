...
from .lendborrowpool import LendBorrowPool
...
def get_buffered(process_dt: date):
    ...
    buffer_table = DefiLlama.LEND_BORROW_POOLS_HISTORICAL.clickhouse_buffer_table()
    return set(
        run_query_oplabs(
            query=f"""
...
def fetch_and_write(session, process_dt, batch: list[str], metadata: LendBorrowPoolsMetadata):
    """Fetch data and write to the ingestion buffer in ClickHouse."""

    pools: list[LendBorrowPool] = []
    for pool in batch:
        pools.append(LendBorrowPool.fetch(pool_id=pool, metadata=metadata, session=session))
    log.info(f"fetched data for a batch of {len(batch)} pools")
...
def copy_to_gcs(process_dt: date, last_n_days: int):
    ...
    writer1 = FromClickHouseWriter(
        dailydata_table=DefiLlama.LEND_BORROW_POOLS_HISTORICAL,
